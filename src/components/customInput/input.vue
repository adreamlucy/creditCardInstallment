  ...
  <div class="input-container" @click.stop="handleClickOpen()">
    <div class="input" @click.stop="handleClickOpen()" ref="input">
      ...
      <span
        ...
        :key="index"
        :class="[{cursor: index === postion}, 
          {'show-separator': type === 'CARD' && (index + 1) % 4 === 0},
          {'show-separator': type === 'ID' && (index === 5 || index === 13)},
          {'show-separator': type === 'MOBILE' && (index === 2 || index === 6)}]"
        @click.stop="handleClickOpen(index)"
      >{{ item }}</span>
      <van-icon v-if="value.length > 0 && postion" class="van-icon van-icon-clear van-field__clear" @click="handleClearValue"></van-icon>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    // 自定义键盘展开事件
    handleClickOpen(index = this.value.length) {
  ...
  created() {
    ...
    vmKeyboard.$on('handleClick', val => {
      ...
      if ((input.value.length  + 1) > input.maxLen) return false
      let isDot = val === '.'
      // 已存在小数点
      if (input.value.indexOf('.') >= 0 && isDot) return false
      if (this.strCharPosition(input.value, 'X') > 0) return false
    ...
    // 自定义键盘点击删除事件
    vmKeyboard.$on('handleClickDelete', () => {
      let input = this.$root.customInput.input
